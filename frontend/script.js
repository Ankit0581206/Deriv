...
async function runSentinelScan() {
    ...
    try {
        const source = new EventSource(`${state.apiUrl}/api/v1/sentinel/scan/stream`);

        source.onerror = function(error) {
            console.error('Sentinel Scan Stream Error or Disconnected');
            elements.sentinelGlobalLoading.classList.add('hidden');
            source.close();
        };

        source.addEventListener("scan_started", function(e) {
            console.log("Sentinel Scan Started...");
        });

        source.addEventListener("mission_complete", function(e) {
            const data = JSON.parse(e.data);
            state.sentinelDetections.push(data);
            renderSentinelDetections([data]); // Render individual detection as it arrives
            updateSentinelStats(state.sentinelDetections);
        });
        
        source.addEventListener("narrative_complete", function(e) {
            // Optional: you can show the narrative data here
            console.log("Narrative complete");
        });

        source.addEventListener("scan_complete", function(e) {
            console.log("Sentinel Scan Completed!");
            elements.sentinelGlobalLoading.classList.add('hidden');
            state.sentinelScans++;
            source.close();
        });
